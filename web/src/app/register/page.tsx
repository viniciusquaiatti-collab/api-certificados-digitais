...
export default function RegisterPage() {
  ...
  // ============================================================
  // 📝 HANDLE REGISTER — cadastro local
  // ============================================================
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      logger.info("HTTP", "Status da resposta", {
      ...
      if (token) {
        ...
        try {
          ...
          logger.warn("AUTH", "Token não decodificável para log");
        }
  ...
  //     O backend detecta se é novo usuário (INSERT) ou
  //     existente (UPDATE/vinculação) automaticamente.
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4">
      ...
      <div
        className="fixed inset-0 z-0 pointer-events-none"
        style={{ background: "radial-gradient(ellipse 60% 50% at 50% 0%, rgba(16,185,129,0.08) 0%, transparent 70%)" }}
      />
        ...
        >
          ...
          <form onSubmit={handleRegister} className="space-y-4" noValidate>
            ...
            <div className="relative">
              ...
              <button
                type="button"
                onClick={() => { setShowPassword(!showPassword); logger.event("INPUT", "toggle senha", { visible: !showPassword }); }}
            ...
            <p className={`text-[11px] transition-colors duration-200 ${
              senha.length > 0 && senha.length < 8 ? "text-amber-400" : "text-gray-600"
            }`}>
            ...
            <button
              type="submit"
              disabled={loading || loadingGoogle}
              onClick={() => logger.event("BTN", "submit clicado")}
              className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-900 disabled:text-emerald-700 disabled:cursor-not-allowed text-black font-semibold py-3 rounded-xl transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] text-sm tracking-wide mt-1"
            >
              {loading ? (
          ...
          <button
            ...
            disabled={loading || loadingGoogle}
            className="w-full flex items-center justify-center gap-3 border border-white/[0.08] hover:border-white/[0.18] bg-white/[0.03] hover:bg-white/[0.06] disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm py-3 rounded-xl transition-all duration-200 group cursor-pointer"
          >
            {loadingGoogle ? (
              <>
          ...
          <p className="text-center text-gray-600 text-xs mt-6">
            Já tem uma conta?{" "}
            <button
              onClick={() => { logger.event("NAV", "→ /login"); router.push("/login"); }}
