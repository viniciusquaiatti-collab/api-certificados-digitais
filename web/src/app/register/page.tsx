    ...
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4">
      ...
      >
        ...
        >
          ...
          {/* GOOGLE REGISTER
              ✅ PREPARADO — mesmo padrão do login.
              🔧 Para ativar: configurar Google OAuth no backend
                 e descomentar a linha no handleGoogleRegister()
          */}
          <button
            type="button"
            onClick={() => {
              logger.event("REGISTER:GOOGLE", "Usuário clicou em Continuar com Google");
              logger.warn("REGISTER:GOOGLE", "Google OAuth ainda não configurado — aguardando integração");
              // TODO: descomentar quando Google OAuth estiver configurado no backend
              // window.location.href = `${API_URL}/api/auth/google`;
            }}
            className="w-full flex items-center justify-center gap-3 border border-white/[0.08] hover:border-white/[0.18] bg-white/[0.03] hover:bg-white/[0.06] text-white text-sm py-3 rounded-xl transition-all duration-200 group cursor-pointer"
          >
